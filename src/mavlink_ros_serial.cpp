...
//__________________________________________________________ARDUINO_____________________________________________
/**
 * @file
...
void convertMavlinkCustomIMUtoROS(mavlink_message_t* message, sensor_msgs::Imu &imu_msg)
{
    ...
    imu_msg.linear_acceleration.x = puppetcopter_imu.xacc; //floor(puppetcopter_imu.xacc * 1000 + 0.5) /1000;
...
void convertROStoMAVLink(const mavlink_ros::controller_out & msg)
{
	...
	mavlink_msg_set_roll_pitch_yaw_thrust_pack(sysid, compid, &msg_t, MAV_AUTOPILOT_GENERIC/*target_system (???)*/, MAV_COMP_ID_IMU/*target_component (???)*/, msg.set_theta, msg.set_phi, msg.set_psi, msg.set_thrust);
...
void mavlinkCallback(const mavlink_ros::Mavlink &mavlink_ros_msg) //Filippo prende un mavlink-ros messaggio e me lo invia dalla porta
{
